...
async function talkOrNull(talkId: string) {
...
export async function getTalkContextRoute(input: {
  auth: AuthContext;
  talkId: string;
}): Promise<{
  statusCode: number;
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    return {
      statusCode: 200,
      body: { ok: true, data: await getTalkContext(input.talkId) },
    };
  });
}
...
export async function setTalkGoalRoute(input: {
  auth: AuthContext;
  talkId: string;
  goalText: string;
}): Promise<{
...
export async function listTalkContextRulesRoute(input: {
  auth: AuthContext;
  talkId: string;
}): Promise<{
  statusCode: number;
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    return {
      ...
      body: {
        ok: true,
        data: { rules: await listTalkContextRules(input.talkId) },
      },
    };
  });
}
...
export async function getTalkStateRoute(input: {
  auth: AuthContext;
  talkId: string;
}): Promise<{
  statusCode: number;
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    return {
      ...
      body: {
        ok: true,
        data: { entries: await listTalkStateEntries(input.talkId) },
      },
    };
  });
}

export async function deleteTalkStateEntryRoute(input: {
  auth: AuthContext;
  talkId: string;
  key: string;
}): Promise<{
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    const deleted = await forceDeleteTalkStateEntry(input.talkId, input.key);
...
export async function createTalkContextRuleRoute(input: {
  auth: AuthContext;
  talkId: string;
  ruleText: string;
}): Promise<{
...
export async function patchTalkContextRuleRoute(input: {
  auth: AuthContext;
  talkId: string;
  ruleId: string;
  ruleText?: string;
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    try {
      const rule = await patchTalkContextRule({
        ruleId: input.ruleId,
        talkId: input.talkId,
...
export async function deleteTalkContextRuleRoute(input: {
  auth: AuthContext;
  talkId: string;
  ruleId: string;
}): Promise<{
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    const deleted = await deleteTalkContextRule(input.ruleId, input.talkId);
...
export async function createTalkContextSourceRoute(input: {
  auth: AuthContext;
  talkId: string;
  sourceType: string;
  title: string;
...
export async function patchTalkContextSourceRoute(input: {
  auth: AuthContext;
  talkId: string;
  sourceId: string;
  title?: string;
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    const source = await patchTalkContextSource({
...
export async function retryTalkContextSourceRoute(input: {
  auth: AuthContext;
  talkId: string;
  sourceId: string;
}): Promise<{
  ...
  return await withUserContext(input.auth.userId, async () => {
    ...
    const existing = await getTalkContextSourceById(
      input.sourceId,
      input.talkId,
    );
    ...
    const source = await markTalkContextSourcePending(
      input.sourceId,
      input.talkId,
    );
    if (!source) return notFoundResponse('Source not found.');
