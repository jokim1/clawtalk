...
3. **Fire-time dependency check.** The scheduler still holds the Talk row lock here; Talk roster/tool writers participate in the same lock so dependency validation and the source-scoped tool snapshot cannot interleave with a concurrent Talk-tool revoke. Verify:
   ...
   - The agent's `model_id` references an enabled `llm_models` row whose provider is also enabled.
...
25. **Disabled model or provider blocks the job.** Set the targeted agent's `model_id` to point at an `llm_models` row with `enabled=false`, or disable that model's provider; next fire blocks with `block_reason='model_disabled'`; no run is inserted. Re-enabling the model/provider (or pointing the agent at an enabled model on an enabled provider) + editing the job clears the block.
