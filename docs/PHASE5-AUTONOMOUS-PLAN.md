...
- **Hung cross-model CLI fallback:** if `/claude review` or `/codex review` hangs, first prove the CLI itself can respond with a trivial smoke prompt, then make two bounded review attempts (full diff and compact/summarized diff) and paste the commands, elapsed time, killed PID/exit code, and any stdout/stderr. A same-model `codex review --uncommitted`, manual skeptic sweep, or gstack fallback may be pasted as supplemental evidence, but it does **not** convert the cross-model gate to PASS. The slice either resumes after the cross-model tool works or hands off with the unresolved review as the blocker.
