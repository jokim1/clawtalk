...
| De-facade | ~10% 🔄 | The dead duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were deleted, and the in-Talk flat-content split-editor was migrated to native blocks (lane-o: `de-facade-readiness.sh` `bodyMarkdown`/`bodyHtml` consumers 93 → 60; only the export/copy lane + `Content` type remain in the webapp). Remaining compat facades still serve live consumers: synthetic threads, runs-with-`threadId`, the flat-content export/copy lane, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard. |
...
The biggest missing work is Talk/Settings Salon completion, remaining Home lifecycle actions, de-facade, live eval hardening, final surface completion, and flat content facade retirement after the export/copy lane moves native (the legacy split-editor consumer is already migrated). Forge remains post-MVP.
...
| 6    | Documents                             | ✅ Native backend API/client path exists for list/detail tabs+blocks+pending edits and edit accept/reject; standalone Documents UI (index + viewer + edit-review console) shipped (PR #557), the in-Talk doc pane consumes the same native tabs/blocks path (PR #560), and the legacy in-Talk split-editor pane is now migrated to that native read+review surface (lane-o). Remaining: flat content facade retirement after the export/copy lane moves. |
...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 89 / 87 / 59 / 106 / 344 | In-Talk split-editor migrated to native (lane-o): the only remaining webapp readers are the export/copy lane (`doc-export.ts`/`CopyExportMenu`) + the shared `Content` type; backend executor edit-ops and accessors still project markdown/html. `de-facade-readiness.sh` `bodyMarkdown\|bodyHtml` consumers: 93 → 60. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Export/copy lane moves to native blocks (Codex lane); backend content routes retire after that. | In progress |
...
`threadId` still appears across `TalkDetailPage.tsx`, `useTalkRunStream.ts`, `talkRunReducer.ts`, `api.ts`, `useTalkSnapshot.ts`, `talkStream.ts`, and `wsCacheRouter.ts`. Native document blocks are consumed by the standalone Documents UI, the in-Talk Documents tab, and now the in-Talk split pane (the `talk` tab reads native blocks via `TalkDocPane`). The only remaining flat-content `bodyMarkdown`/`bodyHtml` readers in the webapp are the export/copy lane (`doc-export.ts`/`CopyExportMenu`) and the shared `Content` type. This work is structural and behavioral, so it should be goal-scoped by facade, not attempted as one large rewrite.
...
- Delete flat content facade after the final consumer (the export/copy lane) moves.
