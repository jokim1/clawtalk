...
| --- | ------------------------------ | --------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 1   | Synthetic threads              | `threadId` is fabricated even though the schema has no threads. Old `/talks/:talkId/threads[...]` REST endpoints are deleted; Talk URL/cache/request contracts are Talk-level; snapshot/run/message DTOs still carry thread identity. | Frontend treats Talk as the conversation and drops `threadId`.         |
...
| -------------------------------------------------- | -------------------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | ------- |
| Synthetic `threadId`                               | 268 / 63 / 33 / 166 / 394 | Old thread REST endpoints and webapp wrappers are deleted. Talk navigation/cache/request contracts are now Talk-level: no `?thread=`, no last-thread localStorage helper, no thread-keyed snapshot cache, and no thread ids in snapshot/messages/chat/cancel/document-create requests. Backend snapshot/run/message compatibility plus frontend Talk snapshot, stream, reducer, and temporary thread rail state still carry synthetic thread identity. | Treat Talk as the conversation boundary; native hydration/messages/runs drop thread identity.                | Client DTO types, stream reducer, snapshot tests, and backend serializers stop exposing `threadId`; route tests cover native Talk-only hydration. | Blocked |
| Runs/messages `threadId` DTO fields                | 528 / 235 / 46 / 293 / 82 | `TalkMessage`/`TalkRun` DTOs, reducers, stream hook, snapshot and tests still carry `threadId` and response-group compatibility.                               | Native run/message DTOs keyed by run id, response group, round, and message id.                              | Client DTO types, stream reducer, snapshot tests, and backend serializers stop exposing `threadId`.                                        | Blocked |
...
`threadId` still appears across `TalkDetailPage.tsx`, `useTalkRunStream.ts`, `talkRunReducer.ts`, `api.ts`, `talkStream.ts`, and backend run/message serializers, although the old `/talks/:talkId/threads[...]` REST endpoints/webapp wrappers and the Talk URL/cache/request consumers are gone. Native document blocks are consumed by the standalone Documents UI, the in-Talk Documents tab, the native export/copy path, and now the in-Talk split pane (the `talk` tab reads native blocks via `TalkDocPane`). Production webapp and backend route greps for flat body fields are now empty. Remaining de-facade work is structural and behavioral, so it should be goal-scoped by facade, not attempted as one large rewrite.
