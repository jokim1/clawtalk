...
> **Status:** live audit snapshot updated 2026-06-08 for the Phase 5 native Documents API unblocker, MVP dry-run CI eval gate, duplicate-route de-facade deletion, flat-content route/projection deletion, Home P2 hardening, and Documents accept/proposal race hardening.
...
| De-facade | ~15% 🔄 | The dead duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were deleted; native block export/copy shipped (PR #561); the in-Talk flat-content split-editor was migrated to native blocks (PR #562); final webapp export/DTO compatibility remnants were removed; definition-only retired legacy `contents`/`content_edits` DB helpers were deleted; the old executor flat-content fallback was retired in favor of the native `document_edits` tool path; Talk doc-pane creation now uses native `POST /api/v1/documents`; old flat-content HTTP route mounts/helpers for `/talks/:talkId/content`, `/threads/:threadId/content`, and `/contents/:contentId[...]` were deleted; and the snapshot body projection was replaced with native `primaryDocument` metadata. `de-facade-readiness.sh` now reports `flat-content-projection live_consumer_count: 0`. Remaining compat facades still serve live consumers: synthetic threads, runs-with-`threadId`, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard. |
...
- Keep the flat content body-projection scout at zero while the remaining synthetic thread, snapshot-version, run-context, policy/tool/connectors, and attachments facades are migrated independently.
