...
The biggest missing work is Talk/Settings Salon completion, remaining Home lifecycle actions, de-facade, live eval hardening, final surface completion, and flat content facade retirement after the remaining backend compatibility routes/accessors and shared rich-text edit ops retire. Forge remains post-MVP.
...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 72 / 91 / 59 / 57 / 213 | Native Documents export/copy moved to block serialization (PR #561), the in-Talk split-editor migrated to native blocks (PR #562), and the final webapp export/DTO compatibility remnants are removed. Production webapp grep for `bodyMarkdown`/`bodyHtml` is empty; remaining hits are backend compatibility routes/accessors, `content-apply-handler`, shared rich-text edit ops, and page-level test fixtures. `de-facade-readiness.sh` now reports 43 flat-content matching lines. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Migrate or delete backend content compatibility routes/accessors and shared rich-text edit ops; then remove compatibility tests once native document tests cover the replacement behavior and the audit proves zero live consumers. | In progress |
...
- Delete flat content facade after the remaining backend compatibility routes/accessors, `content-apply-handler`, shared rich-text edit ops, and compatibility tests move or retire.
