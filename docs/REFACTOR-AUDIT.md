...
| De-facade | ~10% 🔄 | The dead duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were deleted; native block export/copy shipped (PR #561); the in-Talk flat-content split-editor was migrated to native blocks (PR #562); final webapp export/DTO compatibility remnants were removed; and definition-only retired legacy `contents`/`content_edits` DB helpers were deleted. `de-facade-readiness.sh` now reports 33 `bodyMarkdown`/`bodyHtml` matching lines, with zero production webapp hits. Remaining compat facades still serve live consumers: synthetic threads, runs-with-`threadId`, backend/shared flat-content projections/routes, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard. |
...
The biggest missing work is Talk/Settings Salon completion, remaining Home lifecycle actions, de-facade, live eval hardening, final surface completion, and flat content facade retirement after the remaining backend compatibility routes, old executor fallback, and shared rich-text edit ops retire. Forge remains post-MVP.
...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 58 / 73 / 59 / 56 / 198 | Native Documents export/copy moved to block serialization (PR #561), the in-Talk split-editor migrated to native blocks (PR #562), final webapp export/DTO compatibility remnants are removed, and definition-only retired legacy `contents`/`content_edits` DB helpers were deleted. Production webapp grep for `bodyMarkdown`/`bodyHtml` is empty; remaining hits are backend compatibility routes, the old executor fallback, shared rich-text edit ops, and page-level test fixtures. `de-facade-readiness.sh` now reports 33 flat-content matching lines. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Migrate or delete backend content compatibility routes, old executor fallback, and shared rich-text edit ops; then remove compatibility tests once native document tests cover the replacement behavior and the audit proves zero live consumers. | In progress |
...
- Delete flat content facade after the remaining backend compatibility routes, old executor fallback, shared rich-text edit ops, and compatibility tests move or retire.
