...
- Keep the flat content body-projection, run-context fabrication, synthetic thread, connector route, and tool-family readiness scouts at zero while the remaining attachments guard is either implemented over R2-backed storage or removed by product decision.
...
- facade consumers: `attachments_not_available`, old flat content fields, old thread route/query symbols, and duplicate route mounts.
