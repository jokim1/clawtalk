...
The biggest missing work is Talk/Settings Salon completion, remaining Home lifecycle actions, de-facade, live eval hardening, final surface completion, and flat content facade retirement after the legacy split-editor and backend compatibility routes move native. Forge remains post-MVP.
...
| 6    | Documents                             | ✅ Native backend API/client path exists for list/detail tabs+blocks+pending edits and edit accept/reject; standalone Documents UI (index + viewer + edit-review console) shipped (PR #557), the in-Talk doc pane consumes the same native tabs/blocks path (PR #560), and native Documents copy/export serializes blocks directly. Remaining: flat content facade retirement after legacy split-editor and backend compatibility route migration. |
...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 105 / 157 / 59 / 53 / 282 | Legacy Talk split-editor, content API client/types, pending edit surface, rich-text edit ops, stream handling, backend compatibility routes/accessors, and tests still use flattened markdown/html projections. Native Documents copy/export no longer reads these fields. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Legacy split-editor/editor tests move to native blocks or retire; `/api/v1/threads/:threadId/content` and `/api/v1/contents/:contentId` become unused; backend content compatibility tests are replaced by native document tests. | Blocked |
...
`threadId` still appears across `TalkDetailPage.tsx`, `useTalkRunStream.ts`, `talkRunReducer.ts`, `api.ts`, `useTalkSnapshot.ts`, `talkStream.ts`, and `wsCacheRouter.ts`. Native document blocks are consumed by the standalone Documents UI, in-Talk pane, and native copy/export path, but flat content projections still have legacy split-editor, content API, backend route/accessor, and rich-text edit-op consumers. This work is structural and behavioral, so it should be goal-scoped by facade, not attempted as one large rewrite.
...
- Standalone Documents index/detail/edit-review, in-Talk Documents pane, and native Documents copy/export are live over native tabs/blocks; the legacy split-editor and backend content compatibility routes still block flat content facade deletion.
...
- Pending edit accept/reject UX exists in standalone Documents and the in-Talk pane; full authoring editor remains future work.
- Delete flat content facade after the legacy split-editor, content API client/types, and backend compatibility routes/tests move or retire.
