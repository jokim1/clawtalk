...
| De-facade | ~10% 🔄 | The dead duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were deleted; native block export/copy shipped (PR #561); and the in-Talk flat-content split-editor was migrated to native blocks (PR #562). `de-facade-readiness.sh` now reports 45 `bodyMarkdown`/`bodyHtml` matching lines. Remaining compat facades still serve live consumers: synthetic threads, runs-with-`threadId`, backend flat-content projections/routes, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard. |
...
The biggest missing work is Talk/Settings Salon completion, remaining Home lifecycle actions, de-facade, live eval hardening, final surface completion, and flat content facade retirement after the remaining backend compatibility routes/accessors and shared `Content` DTO retire. Forge remains post-MVP.
...
| 6    | Documents                             | ✅ Native backend API/client path exists for list/detail tabs+blocks+pending edits and edit accept/reject; standalone Documents UI (index + viewer + edit-review console) shipped (PR #557), the in-Talk doc pane consumes the same native tabs/blocks path (PR #560), native block export/copy shipped (PR #561), and the legacy in-Talk split-editor pane is now migrated to that native read+review surface (PR #562). Remaining: flat content facade retirement after backend compatibility routes/accessors and the shared `Content` DTO retire. |
...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 74 / 91 / 59 / 55 / 224 | Native Documents export/copy moved to block serialization (PR #561) and the in-Talk split-editor migrated to native blocks (PR #562). Remaining webapp production hits are `webapp/src/lib/api.ts`'s shared `Content` DTO and `webapp/src/lib/doc-export.ts`'s explicitly named legacy projection helper; backend executor edit ops, accessors, and compatibility routes still project markdown/html. `de-facade-readiness.sh` now reports 45 flat-content matching lines. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Retire the legacy export projection helper/tests, migrate or delete backend content compatibility routes/accessors, then remove the shared `Content` DTO fields once the audit proves zero live consumers. | In progress |
...
- Delete flat content facade after the remaining backend compatibility routes/accessors, rich-text edit ops, shared `Content` DTO, and legacy export projection helper/tests move or retire.
