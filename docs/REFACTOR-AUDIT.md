...
| --- | ------------------------------ | --------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 1   | Synthetic threads              | `threadId` is fabricated even though the schema has no threads. Old `/talks/:talkId/threads[...]` REST endpoints are deleted; snapshot/run/message DTOs still carry thread identity. | Frontend treats Talk as the conversation and drops `threadId`.         |
...
`threadId` still appears across `TalkDetailPage.tsx`, `useTalkRunStream.ts`, `talkRunReducer.ts`, `api.ts`, `useTalkSnapshot.ts`, `talkStream.ts`, and `wsCacheRouter.ts`, although the old `/talks/:talkId/threads[...]` REST endpoints and webapp wrappers are gone. Native document blocks are consumed by the standalone Documents UI, the in-Talk Documents tab, the native export/copy path, and now the in-Talk split pane (the `talk` tab reads native blocks via `TalkDocPane`). Production webapp and backend route greps for flat body fields are now empty. Remaining de-facade work is structural and behavioral, so it should be goal-scoped by facade, not attempted as one large rewrite.
...
- Keep the flat content body-projection, run-context fabrication, and tool-family readiness scouts at zero while the remaining synthetic thread, connector, and attachments facades are migrated independently.
