...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 41 / 67 / 59 / 56 / 174 | Native Documents export/copy moved to block serialization (PR #561), the in-Talk split-editor migrated to native blocks (PR #562), final webapp export/DTO compatibility remnants are removed, definition-only retired legacy `contents`/`content_edits` DB helpers were deleted, and the old executor fallback now fails closed while production `apply_content_edit` uses native `document_edits`. Production webapp grep for `bodyMarkdown`/`bodyHtml` is empty; remaining hits are backend content compatibility routes and page-level test fixtures. `de-facade-readiness.sh` now reports 19 flat-content matching lines. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Migrate or delete backend content compatibility routes; then remove compatibility tests once native document tests cover the replacement behavior and the audit proves zero live consumers. | In progress |
