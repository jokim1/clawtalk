...
| 6    | Documents                             | ✅ Native backend API/client path exists for list/detail tabs+blocks+pending edits and edit accept/reject; standalone Documents UI (index + viewer + edit-review console) shipped (PR #557), the in-Talk doc pane consumes the same native tabs/blocks path (PR #560), and the legacy in-Talk split-editor pane is now migrated to that native read+review surface (PR #562). Remaining: flat content facade retirement after the export/copy lane moves. |
...
| Flat content projections `bodyMarkdown`/`bodyHtml` | 89 / 87 / 59 / 106 / 344 | In-Talk split-editor migrated to native (PR #562): the only remaining webapp readers are the export/copy lane (`doc-export.ts`/`CopyExportMenu`) + the shared `Content` type; backend executor edit-ops and accessors still project markdown/html. `de-facade-readiness.sh` `bodyMarkdown\|bodyHtml` consumers: 93 → 60. | Native Documents tabs/blocks/pending edits over `documents`, `doc_tabs`, `doc_blocks`, and `document_edits`. | Export/copy lane moves to native blocks (Codex lane); backend content routes retire after that. | In progress |
