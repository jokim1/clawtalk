...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content (`bodyMarkdown`/`bodyHtml`): native block export/copy shipped (PR #561), the in-Talk split-editor consumer migrated to native (PR #562), the remaining webapp export/DTO compatibility remnants were removed, and definition-only retired legacy DB helpers were deleted; `de-facade-readiness.sh` now reports 33 flat-content matching lines.** Production webapp grep for `bodyMarkdown`/`bodyHtml` is empty; backend compatibility routes/accessors, the old executor fallback, and shared rich-text edit ops still project markdown/html. Next, migrate frontend off synthetic threads/run-context/snapshot/tool/policy/connectors facades; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
