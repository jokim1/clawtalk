...
| 3 | Native Documents | ✅ Standalone Documents page + viewer/edit-review console shipped (PR #557) over `documents`/`doc_tabs`/`doc_blocks`/`document_edits`; accept/proposal writes now serialize to close the same-block supersede race. Remaining: in-Talk doc pane (deferred behind TalkDetail refactor #549); content markdown/html facade deletion after the in-Talk consumer moves. |
