...
| 3 | Native Documents | ✅ Standalone Documents page + viewer/edit-review console (PR #557), in-Talk doc pane (PR #560), and native block copy/export shipped over `documents`/`doc_tabs`/`doc_blocks`/`document_edits`; accept/proposal writes now serialize to close the same-block supersede race. Remaining: content markdown/html facade deletion once the legacy split-editor and backend compatibility routes move. |
