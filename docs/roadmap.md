...
| De-facade | 🔄 Flat-content body projection retired; other facades remain | The duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were removed; those routes now resolve through `mountGreenfieldApiRoutes`. Native Documents export/copy (PR #561), the in-Talk split pane (PR #562), and the webapp export DTO/helper cleanup now read native blocks instead of the flat markdown/html body facade. Definition-only retired legacy `contents`/`content_edits` DB helpers, the old executor flat-content fallback, and the old flat-content HTTP route mounts/helpers for `/talks/:talkId/content`, `/threads/:threadId/content`, and `/contents/:contentId[...]` were removed. Native document creation now uses `POST /api/v1/documents`, and the Talk doc-pane no longer calls the old content create routes. The Talk snapshot now returns native `primaryDocument` metadata instead of projecting document blocks into flat body fields; `de-facade-readiness.sh` reports `flat-content-projection live_consumer_count: 0`. Synthetic threads, runs-with-`threadId`, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard still need native consumers or product decisions before deletion. Readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content:** native block export/copy shipped (PR #561), the in-Talk split-editor consumer migrated to native (PR #562), the remaining webapp export/DTO compatibility remnants were removed, definition-only retired legacy DB helpers were deleted, the old executor fallback now fails closed, Talk doc-pane creation moved to native `POST /api/v1/documents`, the old flat-content HTTP routes/helpers are deleted, and the snapshot body projection is retired. `de-facade-readiness.sh` now reports `flat-content-projection live_consumer_count: 0`. Snapshot compatibility is retired by renaming the outbox high-water field to `eventHighWater`, and the legacy Talk policy `/policy` route/handlers are deleted after grep proved no live client consumers. Next, migrate frontend off synthetic threads/run-context/tool/connectors facades; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
