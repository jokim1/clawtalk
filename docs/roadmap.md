...
| De-facade | 🔎 Readiness scouted | `REFACTOR-AUDIT.md` now has a Phase 5 deletion ledger and repeatable consumer audit. Synthetic threads, runs-with-`threadId`, flat content markdown/html, snapshot compat, policy/tool/connectors facades, duplicate route mounts, and attachment guards all remain blocked by live consumers or active backend ownership. |
