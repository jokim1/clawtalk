...
| Frontend structure | 🔄 Mid-flight | `TalkDetailPage.tsx` is 5,429 LOC, target roughly 2,500. `SettingsPage.tsx` is 1,066 LOC. Talk panels, composer, thread view, reducer, stream hook, and Settings Profile/Tools/OAuth panels are extracted; the Talk tab shell and page-owned controller state remain. The orphaned `TalkLlmSettingsCard.tsx` component has been deleted. |
...
| 2 | Structural cleanup | `TalkDetailPage.tsx` and `SettingsPage.tsx` shrink through behavior-preserving hooks/components; Settings Profile/Tools/OAuth extraction and `TalkLlmSettingsCard.tsx` orphan deletion are complete. |
