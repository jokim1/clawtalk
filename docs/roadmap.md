...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content:** native block export/copy shipped (PR #561), the in-Talk split-editor consumer migrated to native (PR #562), the remaining webapp export/DTO compatibility remnants were removed, definition-only retired legacy DB helpers were deleted, the old executor fallback now fails closed, Talk doc-pane creation moved to native `POST /api/v1/documents`, the old flat-content HTTP routes/helpers are deleted, and the snapshot body projection is retired. `de-facade-readiness.sh` now reports `flat-content-projection live_consumer_count: 0`; the broader audit still has non-facade `Content`/rendering lexical hits that are not markdown/html body compatibility routes or fields. Next, migrate frontend off synthetic threads/run-context/snapshot/tool/policy/connectors facades; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
