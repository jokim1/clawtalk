...
| De-facade | 🔄 Duplicate mounts deleted; content UI consumers migrated | The duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were removed; those routes now resolve through `mountGreenfieldApiRoutes`. Native Documents export/copy (PR #561) and the in-Talk split pane (PR #562) now read native blocks instead of `bodyMarkdown`/`bodyHtml`. Remaining flat-content hits are backend compatibility paths, the shared `Content` DTO, and the explicitly named legacy export projection helper/tests; synthetic threads, runs-with-`threadId`, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard still need native consumers or product decisions before deletion. Readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
