...
| De-facade | 🔄 Flat content, snapshot, policy, run-context, and tool-family readiness facades retired; other facades remain | The duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were removed; those routes now resolve through `mountGreenfieldApiRoutes`. Native Documents export/copy (PR #561), the in-Talk split pane (PR #562), and the webapp export DTO/helper cleanup now read native blocks instead of the flat markdown/html body facade. Definition-only retired legacy `contents`/`content_edits` DB helpers, the old executor flat-content fallback, and the old flat-content HTTP route mounts/helpers for `/talks/:talkId/content`, `/threads/:threadId/content`, and `/contents/:contentId[...]` were removed. Native document creation now uses `POST /api/v1/documents`, and the Talk doc-pane no longer calls the old content create routes. The Talk snapshot now returns native `primaryDocument` metadata instead of projecting document blocks into flat body fields; `snapshotVersion` and the legacy policy route are retired; and run-context now returns native context details or `null` instead of fabricated manifest fields. The old thread REST endpoint family (`GET/POST/PATCH/DELETE /api/v1/talks/:talkId/threads[...]`) and webapp wrappers are deleted. Tool execution no longer falls back to legacy `active_tool_families_json`/`tool_families` storage; it uses explicit run snapshots or native `talk_tools` rows. `de-facade-readiness.sh` reports `flat-content-projection`, `snapshot-compat-version`, `policy-facade-route`, `run-context-fabrication`, and `tool-family-compat` live counts at 0. Synthetic thread identity in snapshot/run/message DTOs, connector facades, and the attachments guard still need native consumers or product decisions before deletion. Readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
