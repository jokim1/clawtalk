...
| De-facade | 🔄 Duplicate mounts and flat-content routes deleted; content UI consumers migrated | The duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were removed; those routes now resolve through `mountGreenfieldApiRoutes`. Native Documents export/copy (PR #561), the in-Talk split pane (PR #562), and the webapp export DTO/helper cleanup now read native blocks instead of `bodyMarkdown`/`bodyHtml`; production webapp grep for those fields is empty. Definition-only retired legacy `contents`/`content_edits` DB helpers, the old executor flat-content fallback, and the old flat-content HTTP route mounts/helpers for `/talks/:talkId/content`, `/threads/:threadId/content`, and `/contents/:contentId[...]` were removed. Native document creation now uses `POST /api/v1/documents`, and the Talk doc-pane no longer calls the old content create routes. `de-facade-readiness.sh` reports 5 remaining flat-content live lines, all in the snapshot `toContentApi` compatibility projection; synthetic threads, runs-with-`threadId`, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard still need native consumers or product decisions before deletion. Readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
...
| 3 | Native Documents | ✅ Standalone Documents page + viewer/edit-review console (PR #557), in-Talk doc pane (PR #560), and native block export/copy (PR #561) shipped over `documents`/`doc_tabs`/`doc_blocks`/`document_edits`; accept/proposal writes now serialize to close the same-block supersede race. **The legacy in-Talk split-editor pane was migrated to native (PR #562): the `talk` tab renders `TalkDocPane` → `TalkDocumentView` over native blocks with read + accept/reject review and no `bodyMarkdown`/`bodyHtml` read; the RichTextEditor/HtmlSourceEditor/PendingEditDocSurface authoring stack was deleted (authoring deferred per spec).** Native `POST /api/v1/documents` now creates the primary document for a Talk/thread, and the Talk doc-pane creation flow uses that route. The final webapp export/DTO remnants are cleaned up, and old flat-content route mounts/helpers are deleted. Remaining: snapshot compatibility still projects native documents as markdown/html content. |
...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content (`bodyMarkdown`/`bodyHtml`): native block export/copy shipped (PR #561), the in-Talk split-editor consumer migrated to native (PR #562), the remaining webapp export/DTO compatibility remnants were removed, definition-only retired legacy DB helpers were deleted, the old executor fallback now fails closed, Talk doc-pane creation moved to native `POST /api/v1/documents`, and the old flat-content HTTP routes/helpers are deleted; `de-facade-readiness.sh` now reports 5 flat-content matching lines.** Production webapp grep for `bodyMarkdown`/`bodyHtml` is empty; old content create client calls and backend content route mounts are gone. Remaining flat-content work is the snapshot `toContentApi` projection plus test fixtures. Next, migrate frontend off synthetic threads/run-context/snapshot/tool/policy/connectors facades; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
