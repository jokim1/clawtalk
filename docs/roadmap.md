...
| De-facade | 🔄 Started | Duplicate Hono route mounts are cleaned up. Synthetic threads, runs-with-`threadId`, flat content markdown/html, snapshot compat, and policy/tool/connectors facades still need native consumers and deletion; readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
