...
| De-facade | 🔄 Duplicate mounts deleted; flat-content split-editor migrated | The duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were removed; those routes now resolve through `mountGreenfieldApiRoutes`. The in-Talk flat-content split-editor consumer is migrated to native blocks (lane-o), leaving only the export/copy lane + the `Content` type on `bodyMarkdown`/`bodyHtml`. Synthetic threads, runs-with-`threadId`, the export/copy flat-content lane, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard still need native consumers or product decisions before deletion; readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
...
| 3 | Native Documents | ✅ Standalone Documents page + viewer/edit-review console (PR #557) and in-Talk doc pane (PR #560) shipped over `documents`/`doc_tabs`/`doc_blocks`/`document_edits`; accept/proposal writes now serialize to close the same-block supersede race. **The legacy in-Talk split-editor pane was migrated to native (lane-o de-facade): the `talk` tab renders `TalkDocPane` → `TalkDocumentView` over native blocks with read + accept/reject review and no `bodyMarkdown`/`bodyHtml` read; the RichTextEditor/HtmlSourceEditor/PendingEditDocSurface authoring stack was deleted (authoring deferred per spec).** Remaining: content markdown/html facade deletion once the **exports/copy** consumers (Codex lane) move off the flat `Content` body. |
...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content (`bodyMarkdown`/`bodyHtml`): the in-Talk split-editor consumer is migrated to native (lane-o); `de-facade-readiness.sh` flat-content consumers dropped 93 → 60, and the only remaining webapp readers are the export/copy lane (`doc-export.ts`/`CopyExportMenu`, Codex) plus the shared `Content` type they need.** Next, migrate frontend off synthetic threads/run-context/snapshot/tool/policy/connectors facades and the export/copy lane; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
