...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content:** native block export/copy shipped (PR #561), the in-Talk split-editor consumer migrated to native (PR #562), the remaining webapp export/DTO compatibility remnants were removed, definition-only retired legacy DB helpers were deleted, the old executor fallback now fails closed, Talk doc-pane creation moved to native `POST /api/v1/documents`, the old flat-content HTTP routes/helpers are deleted, and the snapshot body projection is retired. Snapshot compatibility is retired by renaming the outbox high-water field to `eventHighWater`, the legacy Talk policy `/policy` route/handlers are deleted, and run-context now exposes native context details or `null` instead of fabricated manifest/default fields. `de-facade-readiness.sh` now reports `flat-content-projection`, `snapshot-compat-version`, `policy-facade-route`, and `run-context-fabrication` live counts at 0. Next, migrate frontend off synthetic threads/tool/connectors facades; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
