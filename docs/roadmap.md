...
| Talk execution    | Greenfield core/detail/chat routes, chat enqueue, queue consumer, executor, scheduler sweeps, and run-state hardening are committed against `messages`, `runs`, `agents`, and `talk_agent_snapshots`. Finish API shell cleanup before frontend replacement. |
...
| 4    | API shell/resource decomposition cleanup.                                                                                  | 🔄 Next: greenfield surfaces mounted through small route modules; legacy route collisions retired where covered.          |
