...
| De-facade | 🔄 Duplicate mounts deleted; content UI consumers migrated | The duplicate `worker-app.ts` Hono mounts for sidebar reorder and run-context were removed; those routes now resolve through `mountGreenfieldApiRoutes`. Native Documents export/copy (PR #561), the in-Talk split pane (PR #562), and the webapp export DTO/helper cleanup now read native blocks instead of `bodyMarkdown`/`bodyHtml`; production webapp grep for those fields is empty. Definition-only retired legacy `contents`/`content_edits` DB helpers and the old executor flat-content fallback were removed, reducing the flat-content readiness count to 19. Native document creation now uses `POST /api/v1/documents`, and the Talk doc-pane no longer calls the old content create routes. Remaining flat-content hits are backend compatibility routes and page-level test fixtures; synthetic threads, runs-with-`threadId`, snapshot compat, policy/tool/connectors facades, run-context synthesis, and the attachments guard still need native consumers or product decisions before deletion. Readiness ledger: [DE-FACADE-READINESS.md](./DE-FACADE-READINESS.md). |
...
| 5 | Remaining de-facade | Duplicate Hono mounts are deleted. **Flat content (`bodyMarkdown`/`bodyHtml`): native block export/copy shipped (PR #561), the in-Talk split-editor consumer migrated to native (PR #562), the remaining webapp export/DTO compatibility remnants were removed, definition-only retired legacy DB helpers were deleted, the old executor fallback now fails closed, and Talk doc-pane creation moved to native `POST /api/v1/documents`; `de-facade-readiness.sh` still reports 19 flat-content matching lines.** Production webapp grep for `bodyMarkdown`/`bodyHtml` is empty; old content create client calls are gone, but backend compatibility routes still project markdown/html in production. Next, migrate frontend off synthetic threads/run-context/snapshot/tool/policy/connectors facades; delete each backend facade only after the audit script proves zero live consumers and native tests pass. |
