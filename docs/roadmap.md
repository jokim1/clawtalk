...
| Net-new surfaces | 🔄 Home + New Talk sheet + ⌘K palette shipped | Built Salon-native in PR #550: Home (read-first; lifecycle write actions disabled pending the Home write API), the **New Talk sheet** (name-before-create over `createTalk`), and the **⌘K command palette** (nav + Settings tabs + Talks quick switcher). Remaining: standalone Agents page/profile (unblocked, next), native Documents page/editor (blocked — no `/documents` routes), Archive (blocked — no restore route), settings gaps (collision with `codex/settings-decomp`), and Forge (post-MVP). |
...
| 6 | Product surface completion | ✅ New Talk sheet + ⌘K command palette shipped (PR #550). Remaining: standalone Agents page/profile (next), Archive (needs a restore route), settings gaps (Codex-owned decomp), workspace-member management. |
