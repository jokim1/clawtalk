...
- [ ] **Message attachments (deferred):** when prioritized, add the R2-backed DB rows, `/talks/:id/attachments` API, composer upload surface, and chat-attachment vision path in one slice. The v1 baseline deliberately has no chat-message attachment API or composer affordance; context file uploads remain the supported source-ingestion path.
...
| **Compatibility facade drag.**                    | The greenfield backend is live, and the tracked compatibility facades now have deletion scouts. Keep those scouts green; if a future product slice needs an old-shaped contract, reintroduce it only as a native API with explicit tests.                                                                                       |
