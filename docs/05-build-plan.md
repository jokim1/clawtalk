...
### Step 1 · Write the single greenfield migration `0037_clawtalk_greenfield.sql`
...
| **Schema migration size.** | The single `0037_clawtalk_greenfield.sql` is large but executes against disposable local data per D0. Verify with the §11 §14 invariant suite before deploying to staging; a green suite is the gate, not "looks OK." |
