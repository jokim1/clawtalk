...
> **Baseline status (2026-05-30):** the schema reference is written and locally validated; see [`docs/canonical-greenfield-migration.sql`](./canonical-greenfield-migration.sql). That file remains a docs-side reference because it was authored as a destructive drop/create script for the old migration stream. The implementation branch should convert it into a pure final-state baseline at `supabase/migrations/0001_clawtalk_greenfield.sql`: create all final tables directly, archive/remove the old active migration stream, and reset/recreate the Supabase database. See [REFACTOR-OVERVIEW.md §14](./REFACTOR-OVERVIEW.md) for the cutover strategy.
...
| **Schema baseline size.** | The fresh baseline is large, but it applies to an empty/reset Supabase database per D0. Generate it from `11-data-model.md` and [`docs/canonical-greenfield-migration.sql`](./canonical-greenfield-migration.sql), then verify with the §11 §14 invariant suite before deploying to staging; a green suite is the gate, not "looks OK." |
| **Cutover gap.** | The baseline removes the legacy schema that current `src/` and tests target. Landing it without the matching src/ rewrite breaks main. Resolved by holding the active baseline out of `supabase/migrations/` until §05 Phases 2–12 rewrites are ready — at which point the branch replaces the old active migration stream with `supabase/migrations/0001_clawtalk_greenfield.sql`. See REFACTOR-OVERVIEW §14. |
