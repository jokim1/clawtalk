...
1. **Human visual verification of the shell + Slice 1** (blocking before more frontend). Joseph runs the dev stack and confirms the shell renders, the workspace name shows top-left, and the Context tab no longer shows a "State" card. (Local login needs the `.dev.vars` `CLAWTALK_ALLOWED_ORIGINS` fix + a real Google OAuth — see [[greenfield-cutover-active]].) Exercising the *switcher* needs a second workspace; there is intentionally **no create-workspace flow**.
