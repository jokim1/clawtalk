...
3. **Iterate other low-coupling seams.** The composer is last (30+ tangled deps). `SavedSourcesPanel` stays pinned to lifted `contextSources` until the composer/sources untangle (which is also when `['talk-context', talkId]` React-Query ownership can be introduced cleanly — the bundled `getTalkContext` returns sources, which the composer reads, so an RQ key can't be added without touching the composer). **Apply the Slice 2 lesson: extracted panels should be presentational with page-owned state unless the panel manages its own independently-fetched data (cf. `TalkToolsPanel`).**
...
- **Next big frontend decomposition slice = the Jobs tab → `TalkJobsPanel`.** It is the largest remaining inline tab (`currentTab === 'jobs'`, ~7106–7623) and is **fully self-contained** (all jobs state + the derived memos `selectedJob`/`hasUnsavedJobChanges`/`jobToolOptions`/`jobToolFamilyEnabled`/`jobWebEnabled` + the 13 handlers + 4 fetch fns + the 8 job-only pure helpers at ~382–510 are used nowhere else). **Apply the Slice 2 lesson:** the editing state that currently persists across tab switches (`jobDraft`, `creatingJob`, `selectedJobId`) must stay **page-owned** (passed as props) — the panel self-fetches the *data* (`talkJobs`/`selectedJobRuns`/`jobToolsState` + flags/status) like `TalkToolsPanel`, but a self-contained panel that also owned the drafts would reset them on tab-leave (the exact regression codex caught 4×). ~900 lines to relocate; budget a fresh session.
