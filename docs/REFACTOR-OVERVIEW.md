...
| ⏸️ Staged         | Legacy context/runtime retirement slice: final schema provider replay privacy, immutable snapshot provider/model identity, source UUID refs, fail-closed retired executor, greenfield history replay scoping, and job snapshot fix for disabled non-target roster agents                                    | Fully staged and locally verified; commit is blocked only on rerunning GStack Review after Codex CLI quota resets                       |
| 🔄 Active next    | Finish the staged slice's review gate, then commit it before starting frontend rewrite                                                                                                                                                                                                                     | See [IMPLEMENTATION-HANDOFF.md](./IMPLEMENTATION-HANDOFF.md)                                                                            |
...
| **4**  | Chat → executor → queue consumer → outbox → DO streaming end-to-end                                                                           | ⏸️ backend runtime retirement slice staged; local verification + Claude clean, pending GStack Review rerun before commit       |
...
- API shell/resource decomposition — greenfield route modules and their Hono mount layer now cover `/me`, workspace/folders/talk CRUD, roster read/write, the legacy policy facade, talk tools, snapshot/detail/content/thread/document-edit/context/jobs compatibility, chat enqueue/cancel, scheduler Path A, and connector/tool compatibility. The current staged slice retires the remaining legacy context/runtime execution surface; do not start the frontend Talk rewrite until that slice is committed.
