...
| 🔄 Active next    | Retire remaining legacy API collisions: resources/connector bindings; then finish the Phase 9 scheduler rewrite over final jobs                                   | Next slices                                                                                                                  |
...
| **9**  | Jobs: scheduler.ts single-txn claim + queue-consumer atomic claim + UI + inbox emit                                                           | 🔄 compatibility CRUD/run-now committed; scheduler Path A/B + document-append output next |
...
2. **Port resources/connector bindings and the remaining jobs scheduler/runtime paths to greenfield contracts.**
