...
| Flat content `bodyMarkdown`/`bodyHtml` | 19 | `src/clawtalk/web/routes/greenfield-detail.ts`, `src/clawtalk/web/routes/greenfield-api.ts` | Native `documents`/`doc_tabs`/`doc_blocks` plus `document_edits` CAS flows. | Backend content compatibility routes are unused and route tests cover native document replacements; compatibility tests are removed after the audit proves zero live consumers. |
