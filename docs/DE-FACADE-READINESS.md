...
|---|---:|---|---|---|
| Synthetic `threadId` | 216 | The old thread REST endpoints and webapp wrappers for `GET/POST/PATCH/DELETE /api/v1/talks/:talkId/threads[...]` are gone. Talk routing/cache/request contracts are now Talk-level: no `?thread=` links, no last-thread localStorage helper, no thread-keyed snapshot cache, and no thread ids in snapshot/messages/chat/cancel/document-create requests. Remaining evidence is snapshot active-thread hydration, Talk UI's temporary one-conversation rail bridge, run/message DTOs, stream events, and backend `syntheticThreadId` serialization. | Treat Talk as the conversation; runs/messages/content use talk/run/document ids without fabricated thread identity. | `webapp/src/lib/api.ts` has native talk/run/message DTOs; Talk UI removes the temporary thread rail bridge; backend no longer calls `syntheticThreadId`; tests use talk/run/document ids; scout count is zero outside historical docs. |
...
3. Talk native DTOs next: old thread REST endpoints and Talk URL/cache/request consumers are gone; drop remaining synthetic thread identity from API client DTOs, stream reducer, snapshot active-thread bridge, and TalkDetail thread rail state before backend removal.
