...
The native connector routes write the final connector tables directly: `/api/v1/workspace/connector-channels` accepts Slack channels, `/api/v1/workspace/connector-sources` accepts Google Docs/Sheets sources, `GET /api/v1/talks/:talkId/connector-bindings` lists linkable Talk connectors, `PUT|DELETE /api/v1/talks/:talkId/channel-bindings/:channelId` and `PUT|DELETE /api/v1/talks/:talkId/source-bindings/:connectorId` toggle per-Talk connector bindings, and `/api/v1/talks/:talkId/resources` writes Talk Drive resource bindings. Google account credential routes (`/api/v1/me/google-account*`) require explicit workspace scope via `workspaceId`; picker-token may instead use `talkId` so the server resolves the credential workspace from the Talk before minting a Google Picker session. Google Docs/Sheets connector sources are config-only workspace sources: they can be linked to a Talk without a connector secret; actual Google API execution still uses the acting user's `google_tools` OAuth credential.
...
| Endpoint                                 | Purpose                                                                                                                                             |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /talks/:id/connector-bindings`      | Bindings for this Talk only                                                                                                                         |
| `PUT /talks/:id/channel-bindings/:id`    | Link a workspace Slack channel to this Talk. Requires an authorized workspace connector; 409 `CONNECTOR_NOT_AUTHORIZED` otherwise.                  |
| `DELETE /talks/:id/channel-bindings/:id` | Remove a Slack channel binding from this Talk.                                                                                                      |
| `PUT /talks/:id/source-bindings/:id`     | Link a workspace connector source to this Talk. Config-only Google Docs/Sheets sources are considered linkable once a workspace admin creates them. |
| `DELETE /talks/:id/source-bindings/:id`  | Remove a connector source binding from this Talk.                                                                                                   |
