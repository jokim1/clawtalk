...
These documents are **not current**. They mostly describe the retired **"ClawRocket"** architecture (a NanoClaw-derived fork on **SQLite + Docker containers + Telegram/WhatsApp + systemd**) that predates the rename to ClawTalk and the cloud port to **Cloudflare Workers + Supabase Postgres**. They are kept for historical reference and for the occasional durable insight, but an implementer should treat the canonical set in [`../`](../) (docs `01–12`, `README.md`, `REFACTOR-OVERVIEW.md`, `IMPLEMENTATION-READINESS.md`, and `roadmap.md`) as the source of truth.
...
| `SECURITY.md` | ClawRocket security model (container isolation, SQLite secrets, no RLS). | Superseded by [`../SECURITY.md`](../SECURITY.md). |
